    ...
    UUPSUpgradeable
{
    ...
    mapping(address seller => mapping(uint256 number => OrderStatus))
        private s_processing;
    ...
    function mscPurchase(
        MarketStorage calldata marketStorage,
        ...
        if (
            s_processing[marketStorage.maker][marketStorage.number] !=
            OrderStatus.Listing
        ) {
            revert MscMarketV1__OrderIsProcessing();
        }
        s_processing[marketStorage.maker][marketStorage.number] = OrderStatus
            .Sold;
        ...
        (bool success, ) = marketStorage.maker.call{
            value: marketStorage.price - computeFee(marketStorage.price)
        }("");
    ...
    function mscBatchPurchase(
        MarketStorage[] calldata marketStorages,
        bytes[] calldata signatures,
        uint256 totalPrice
    ) external payable nonReentrant {
        if (!s_featureIsEnabled["buy"])
            revert MscMarketV1__FeatureDisabled("buy"); // check if buy feature is enabled
        ...
        if (marketStorages.length != signatures.length)
            revert MscMarketV1__LengthNotEqual(); // check the length
        for (uint256 i = 0; i < marketStorages.length; i++) {
            ...
            if (
                s_processing[marketStorages[i].maker][
                    marketStorages[i].number
                ] != OrderStatus.Listing
            ) continue;
            if (
                !_verifySign(
            ...
            (bool success, ) = marketStorages[i].maker.call{
                value: marketStorages[i].price -
                    computeFee(marketStorages[i].price)
            }("");
            if (!success) {
                // if not suceess, store the failure order
                s_failureOrder[marketStorages[i].maker] =
                    marketStorages[i].price -
                    computeFee(marketStorages[i].price);
            } else {
    ...
    function getFeatureStatus(
        string memory feature
    ) public view returns (bool isEnabled) {
    ...
    ) public view returns (uint256 failureAmount) {
    ...
    function getOrderStatus(
        ...
        uint256 number
    ) public view returns (OrderStatus status) {
